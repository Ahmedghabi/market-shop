...
import { getStoredAccessToken, isAccessTokenExpired } from './getStoredAccessToken';
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      try {
        ...
        if (parsedUser.accessToken && getStoredAccessToken()) {
          setUser(parsedUser);
        } else {
  ...
  const getAccessToken = useCallback(() => {
    ...
    return isAccessTokenExpired(user.accessToken) ? null : user.accessToken;
  }, [user]);
