...
type QuotaDefinition = { id: string; code: string; name: string; description?: string | null; unit?: string | null; category?: string | null; isActive: boolean };
type QuotaForm = { code: string; name: string; description: string; unit: string; category: string; isActive: boolean };
...
const emptyQuotaForm: QuotaForm = { code: '', name: '', description: '', unit: '', category: '', isActive: true };
...
export function ExtensionsAdminPanel({ getAccessToken }: { getAccessToken: () => string | null }) {
  ...
  const openEditQuota = (quota: QuotaDefinition) => {
    ...
    setQuotaForm({ code: quota.code, name: quota.name, description: quota.description ?? '', unit: quota.unit ?? '', category: quota.category ?? '', isActive: quota.isActive });
                  ...
                  <div key={ext.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 12px', border: '1px solid var(--bo-border)', borderRadius: 8 }}>
                    <div>
                      ...
                      <div style={{ fontSize: 12, color: 'var(--bo-text-secondary)' }}>
                        {ext.type} {ext.targetCode ? `→ ${ext.targetCode}` : ''} {ext.value ? `(+${ext.value})` : ''} — {ext.priceTnd} TND{ext.durationMonths ? ` / ${ext.durationMonths} mois` : ' (permanent)'}
                      </div>
                    </div>
        ...
        <div style={{ display: 'grid', gap: 14 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <FormField label="Type" required>
              <Select value={extForm.type} onChange={(e) => setExtForm({ ...extForm, type: e.target.value })}>
            ...
            <FormField label="Code cible" hint="Code du quota / module / thème visé">
              <Input value={extForm.targetCode} onChange={(e) => setExtForm({ ...extForm, targetCode: e.target.value })} placeholder="max_products" />
            </FormField>
          </div>
