...
export function FrontOfficePage({ getAccessToken }: { getAccessToken: () => string | null }) {
  ...
  const storefrontUrl = useMemo(
    () => (boutique?.slug ? `/boutiques/${boutique.slug}` : null),
    [boutique?.slug],
  );
  ...
  async function handleThemeSelect(code: string) {
    ...
    try {
      ...
      const selected = themes.find((t) => t.code === code);
      setForm((f) => ({
        ...f,
        theme: code,
        primaryColor: selected?.colorPalette?.primary ?? f.primaryColor,
        accentColor: selected?.colorPalette?.accent ?? f.accentColor,
        backgroundColor: selected?.colorPalette?.background ?? f.backgroundColor,
        fontFamily: selected?.colorPalette ? f.fontFamily : f.fontFamily,
      }));
