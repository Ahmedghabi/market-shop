...
final class ExtensionProcessor implements ProcessorInterface
{
    ...
    private function validateTarget(ExtensionInput $data): void
    {
        $targetCode = trim((string) ($data->targetCode ?? ''));
        if ('service' === $data->type) {
            $data->targetCode = '' === $targetCode ? null : $targetCode;

            return;
        }

        if ('' === $targetCode) {
            throw new BadRequestHttpException('Sélectionnez la cible de cette extension.');
        }

        $exists = match ($data->type) {
            'module' => null !== $this->modules->findOneByCode($targetCode),
            'theme' => null !== $this->themes->findOneByCode($targetCode),
            'quota_boost' => null !== $this->quotas->findOneByCode($targetCode),
            default => false,
        };

        if (!$exists) {
            throw new BadRequestHttpException(sprintf('Cible inconnue pour cette extension : %s.', $targetCode));
        }
