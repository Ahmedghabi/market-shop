...
final class SeedExtensionsCommand extends Command
{
    ...
    {
        $existing = $this->em->getRepository(Extension::class)->findAll();
        if (count($existing) > 0) {
            $output->writeln('Extensions already seeded ('.count($existing).' found).');
        ...
        $output->writeln(sprintf('Seeded %d extensions.', count(self::EXTENSIONS)));
